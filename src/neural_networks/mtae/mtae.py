...
from neural_networks.st_mem.st_mem import TransformerBlock, ViT
...
__all__ = ["MTAE", "MTAEConfig"]


def get_1d_sincos_pos_embed(embed_dim: int, grid_size: int, temperature: float = 10000, cls_embed: bool = False):
...
class MTAE(nn.Module):
    def __init__(self, cfg: MTAEConfig):
        super().__init__()
        self.cfg = cfg
        seq_len = cfg.seq_len
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_1d_sincos_pos_embed(self.encoder.pos_embedding.shape[-1], self.num_patches, cls_embed=True)
        self.encoder.pos_embedding.data.copy_(pos_embed.float().unsqueeze(0))
        self.encoder.pos_embedding.requires_grad = False

        decoder_pos_embed = get_1d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], self.num_patches, cls_embed=True)
        self.decoder_pos_embed.data.copy_(decoder_pos_embed.float().unsqueeze(0))

        # timm's trunc_normal_(std=.02) is effectively normal_(std=0.02) as cutoff is too big (2.)
        torch.nn.init.normal_(self.encoder.cls_embedding, std=0.02)
